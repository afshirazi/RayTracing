...
use crate::{light::Light, math::Vec3, objects::{Object, RayOps}};
...
impl Camera {
    pub fn new(eye: Vec3, up: Vec3, look_at: Vec3, fov_y: f64) -> Self {
        Camera {
            eye,
            up,
            look_at,
    ...
    pub fn render(&self, objects: &Vec<Object>, lights: &Vec<Light>, img: &mut RgbImage) {
        for (x, y, pixel) in img.enumerate_pixels_mut() {
            let ray = self.get_ray(x, y, 1600, 900);
            let px_color = Self::get_color(&ray, &self.eye, objects, lights, None, 5);
            *pixel = Rgb([
    ...
    fn get_ray(&self, x: u32, y: u32, w: u32, h: u32) -> Vec3 {
